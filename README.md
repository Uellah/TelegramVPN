# Telegram Mini App — VPN Dashboard

Telegram Mini App с мониторингом серверов в реальном времени: CPU, память, сеть, подключения + красивые графики.

## Возможности

- 📊 **Мониторинг в реальном времени** — CPU, память, uptime, активные подключения
- 📈 **Графики CPU** — история нагрузки процессора с Chart.js
- 🌐 **Скорость сети** — живая статистика RX/TX (bytes/sec + total)
- 🎨 **Адаптивный дизайн** — от mobile до desktop (градиенты, анимации, glassmorphism)
- 🔐 **Безопасная передача данных** — агент на локальном ПК пушит метрики через API_KEY
- ⚡ **Prometheus-ready** — поддержка node_exporter для production-метрик

## Требования

- Node.js 18+
- Telegram-бот (создать через [@BotFather](https://t.me/botfather))

## Локальный запуск

1. **Клонируйте и установите зависимости:**

```bash
cd TelagramVPN
npm install
```

2. **Создайте `.env` из примера:**

```bash
cp .env.example .env
```

3. **Заполните `.env`:**

```
BOT_TOKEN=ваш_токен_от_BotFather
WEBAPP_URL=https://ваш-домен.ngrok.io
PORT=3000
```

> Для локальной разработки Mini App нужен HTTPS. Используйте [ngrok](https://ngrok.com): `ngrok http 3000` — возьмите HTTPS-URL в `WEBAPP_URL`.

4. **Настройте Mini App в BotFather:**

- Отправьте `/mybots` → выберите бота → Bot Settings → Configure Mini App
- Укажите URL дашборда: `https://ваш-домен/dashboard`

5. **Запустите сервер:**

```bash
npm run dev
```

Откройте бота в Telegram, отправьте `/start` и нажмите «Открыть приложение».

## Деплой на Render

1. **Создайте аккаунт на [Render](https://render.com).**

2. **New → Web Service**, подключите репозиторий (GitHub/GitLab).

3. **Настройки:**
   - **Build Command:** `npm install && npm run build`
   - **Start Command:** `npm start`
   - **Environment:** Node

4. **Environment Variables (секреты):**
   - `BOT_TOKEN` — токен бота от BotFather
   - `WEBAPP_URL` — URL сервиса Render (например `https://your-app.onrender.com`)
   - `USE_WEBHOOK=true` — обязательно на Render, иначе конфликт 409 при редеплое
   - `PORT` — Render задаёт сам, можно не указывать

5. ** deploy** — Render запустит сборку и деплой.

6. **Настройте Mini App в BotFather:**
   - Bot Settings → Configure Mini App → укажите `https://your-app.onrender.com/dashboard`

## Mini App (Dashboard)

React-дашборд — Mini App. Открывается по кнопке «Открыть приложение» при `/start`.

**Разработка:**
```bash
npm run dev
```
Сервер: http://localhost:3000, дашборд: http://localhost:5173 (Vite)

**Продакшен:** Build command: `npm install && npm run build`

## Структура проекта

```
├── server.js         # Express + API + Bot
├── src/              # React Mini App (дашборд)
├── index.html        # Vite entry
├── vite.config.js
├── .env
└── package.json
```

## Отправка статистики с локального компа

**На твоём KDE Neon:**

1. Создай `.env` в папке проекта:
```env
SERVER_URL=https://telegramuellahbot.onrender.com
API_KEY=твой-секретный-ключ
INTERVAL_SEC=5
```

2. Запусти агента:
```bash
npm run agent
```

Агент каждые 5 секунд отправляет статистику (CPU, память, сеть) на Render. Mini App показывает данные твоего компа.

**Трекер активности (клавиши и клики):**  
Агент может отправлять статистику нажатий и кликов, если в папке проекта есть файл `activity.json` (или путь из `ACTIVITY_FILE`). Формат:

```json
{
  "keysTotal": 1234,
  "clicksTotal": 567,
  "keysPerMin": 12,
  "clicksPerMin": 5,
  "keysByLetter": { "a": 10, "b": 5, " ": 20 }
}
```

Агент читает файл при каждой отправке.

**Режим «из системы» (KDE Neon / Linux) — фоновая статистика по реальному использованию:**  
Запусти сервис с перехватом evdev (`/dev/input`). Нужны права на чтение input: `sudo usermod -aG input $USER` (затем выйти из сессии и зайти снова).

```bash
npm run activity:system
```

или в `.env`: `ACTIVITY_SOURCE=system`, затем `npm run activity`.

Сервис сам находит клавиатуры и мыши, пишет в `activity.json` все нажатия и клики. Статистика по клавишам — в `keysByLetter`. Данные сохраняются каждые ~2 с и при накоплении событий. Просмотр счётчиков: http://localhost:3999/api/activity.

**Режим веб-тестера (ручная проверка клавиатуры):**

```bash
npm run activity
```

Открой http://localhost:3999 — страница теста: фокус в области, нажатия и клики пишутся в `activity.json`. Порт — `ACTIVITY_PORT` в `.env`. В дашборде появятся блок «Моя активность», график по времени, счётчики и разбивка по буквам.

**На Render добавь переменную:**
```
API_KEY=твой-секретный-ключ
```

## API

- `POST /api/me` — валидация initData Telegram
- `POST /api/stats/report` — приём статистики от агента (требует X-API-Key)
- `GET /api/stats` — возврат последней статистики
- `GET /api/servers` — список серверов
